...
int replaceByte(int x, int n, int c) {
  /* Using the following mask, modify the nth byte of x to 0, and then use OR operation with appropriately shifted c) */
  int shift_num = n << 3;
  int mask = ~(0xFF << shift_num); // ~ has more priority than <<.
  return (x & mask)|(c << shift_num);
}
...
int leftBitCount(int x) { 
  ...
  int upper16_con, upper8_con, upper4_con, upper2_con, upper1_con, LSB_exception;
  /* 
   * Using binary search, we'll find the consecutive 1 from leading 1 of each interval(size 16 -> 8 -> 4 -> 2 -> 1 + LSB_exception) 
   * Each time we will check whether the left (32 - cursor) bits are all 1 or not.
   */
  int cursor = 16;

  /*
   * For less operators, I changed the problem to count the number of consecutive 0's from left.
   */
  
  x = ~x;

  upper16_con = !(x >> cursor); // if x(inverted one) has 16 consecutive 0 from leading 0, upper16_con is 1, and otherwise returns 0.
  cursor = cursor + 8 + (~(upper16_con + minus1) & minus16); // add 8 to the cursor if upper16_con is 1, and otherwise add -8 to the cursor.
