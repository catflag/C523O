/*
 * mm.c - implementation using segregated free list / best fit searching.
 * 20160759 Dongpyeong Seo bludrimjr@kaist.ac.kr / 2016 Yusung Sim sys5867@kaist.ac.kr
 * 
...
#define DEBUG 0 // for submission, all do_debug() are now commented.
#define VERBOSE 0
...
static void do_debug()
{
    if(DEBUG)
    {
        ...
        if(mm_check() != 1) // 1 -> check passed
        {
            fprintf(stderr, "do_debug : mm_check is weird.\n");
            exit(1);
        }
    }
}
    ...
    else
    {
        ...
        if(isPrevValid(old_chunkptr))
        {
            ...
            if(GET_C(prev_chunk) == FREE)
            {
                ...
                /* it does not word for mysterious reason... but memmove works.
                if(prev_chunksize + old_chunksize >= req_chunksize)
                {
                    ...
                    for(unsigned int i = prev_chunksize; i < req_datasize; ++i)
                    {
                        *cursor = *(cursor + i);
                    }
...
// mm_check : heap consistency checker. call exit(errno) when inconsistency found / return 1 when heap is consistent.
int mm_check() {
    ...
    while (ptr < mm_brk) {
        ...
        if (!(is_first || NEXTCHK(prev) == ptr)) {
            fprintf(stderr, "NEXTCHK(prev) != ptr\n");
            exit(1);
        }

        // 2) is next's prev is current?
        if (!(is_last || PREVCHK(next) == ptr)) {
            fprintf(stderr, "PREVCHK(next) != ptr\n");
            exit(2);
        }
        ...
        if (cur_c == FREE) {
            ...
            if (!is_first && prev_c == FREE) {
                fprintf(stderr, "prev == FREE\n");
                exit(3);
            }

            // 2) current and next if FREE
            if (!is_last && next_c == FREE) {
                fprintf(stderr, "next == FREE\n");
                exit(4);
            }
        }
        ...
        if (cur_size != *(unsigned int *)FOOTERP(ptr)) {
            fprintf(stderr, "ptr's size != ptr's prev_size (footer value)\n");
            exit(5);
        }

        // 4. check consistency of prev_size
        if (!(is_first || prev_size == GET_PREVSIZE(ptr))) {
            fprintf(stderr, "prev's size != ptr's prev_size\n");
            exit(6);
        }

        if (!(is_last || cur_size == GET_PREVSIZE(next))) {
            fprintf(stderr, "ptr's size != next's prev_size");
            exit(7);
        }
    ...
    for (index = 0; index < 10; ++index) {
        ...
        while (1) {
            ...
            if (ptr < first_chunk || ptr >= mm_brk) {
                fprintf(stderr, "ptr out of heap range\n");
                exit(8);
            }
            int cur_size = GET_SIZE(ptr);
            int cur_c = GET_C(ptr);
    ...
    if (implicit_free_chunks != explicit_free_chunks) {
        fprintf(stderr, "Number of free chunks do not match\n");
        exit(10);
    }
    return 1;
}
